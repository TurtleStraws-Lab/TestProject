...
// Helper: calculate Gaussian probability
    ...
    for (auto& row : X) {
        ...
        for (size_t i = 0; i < model.classes.size(); ++i) {
            ...
            for (size_t j = 0; j < row.size(); ++j)
                prob += std::log(gaussian_prob(row[j], model.means[i][j], model.variances[i][j]));
...
double macroF1_gnb(const std::vector<int>& y_true,
    ...
    for (size_t i = 0; i < y_true.size(); ++i) {
        if (y_true[i] == y_pred[i]) tp[y_true[i]]++;
        else {
            fp[y_pred[i]]++;
            fn[y_true[i]]++;
        }
